...
# -*- coding: utf-8 -*-
"""
Comparison between PyDelt and FinDiff for numerical differentiation.

This script compares the accuracy and performance of PyDelt's differentiation methods
with FinDiff's finite difference methods for computing derivatives of known test functions.
"""
...
def evaluate_methods(x, y, x_eval, true_derivative, noise_level=0.0, order=1):
    """
    ...
    # For FinDiff, we need to handle the evaluation points differently
    # First, we'll evaluate at the original points using FinDiff
    
    # FinDiff with accuracy order 2 (standard)
    start_time = time.time()
    # Calculate the spacing
    dx = x[1] - x[0]
    # Create the derivative operator
    d_dx = FinDiff(0, dx, order, acc=2)
    # Apply to the noisy data
    fd_values_acc2 = d_dx(y_noisy)
    
    # Since FinDiff only computes derivatives at the original grid points,
    # we need to interpolate to get values at x_eval
    from scipy.interpolate import interp1d
    fd_interp_acc2 = interp1d(x, fd_values_acc2, bounds_error=False, fill_value="extrapolate")
    fd_eval_acc2 = fd_interp_acc2(x_eval)
    
    times['FinDiff (Accuracy 2)'] = time.time() - start_time
    results['FinDiff (Accuracy 2)'] = np.abs(fd_eval_acc2 - true_values)
    
    # FinDiff with accuracy order 4 (higher accuracy)
    start_time = time.time()
    d_dx_acc4 = FinDiff(0, dx, order, acc=4)
    fd_values_acc4 = d_dx_acc4(y_noisy)
    fd_interp_acc4 = interp1d(x, fd_values_acc4, bounds_error=False, fill_value="extrapolate")
    fd_eval_acc4 = fd_interp_acc4(x_eval)
    times['FinDiff (Accuracy 4)'] = time.time() - start_time
    results['FinDiff (Accuracy 4)'] = np.abs(fd_eval_acc4 - true_values)
...
def run_comparison(test_name, func, deriv_func, second_deriv_func=None, 
    ...
    for noise_level in noise_levels:
        errors, times = evaluate_methods(x, y, x_eval, deriv_func, noise_level=noise_level, order=1)
    ...
    if second_deriv_func is not None:
        ...
        for noise_level in noise_levels:
            errors, times = evaluate_methods(x, y, x_eval, second_deriv_func, noise_level=noise_level, order=2)
...
def plot_comparison(test_name, func, deriv_func, x_range=(-5, 5), num_points=100, noise_level=0.05):
    ...
    # FinDiff calculations
    dx = x[1] - x[0]
    
    # FinDiff with accuracy order 2
    d_dx_acc2 = FinDiff(0, dx, 1, acc=2)
    fd_values_acc2 = d_dx_acc2(y_noisy)
    
    # FinDiff with accuracy order 4
    d_dx_acc4 = FinDiff(0, dx, 1, acc=4)
    fd_values_acc4 = d_dx_acc4(y_noisy)
    
    # Interpolate FinDiff results to evaluation points
    from scipy.interpolate import interp1d
    fd_interp_acc2 = interp1d(x, fd_values_acc2, bounds_error=False, fill_value="extrapolate")
    fd_eval_acc2 = fd_interp_acc2(x_eval)
    ...
    plt.plot(x_eval, fd_eval_acc2, label='FinDiff (Accuracy 2)')
    plt.plot(x_eval, fd_eval_acc4, label='FinDiff (Accuracy 4)')
    plt.title(f'{test_name} - First Derivative Comparison')
    plt.legend()
    plt.grid(True)
    
    plt.tight_layout()
    plt.savefig(f'../output/{test_name.lower().replace(" ", "_")}_findiff_comparison.png')
    plt.close()
