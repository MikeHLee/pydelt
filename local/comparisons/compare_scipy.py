...
# -*- coding: utf-8 -*-
"""
Comparison between PyDelt and SciPy interpolation methods for numerical differentiation.

This script compares the accuracy of PyDelt's interpolation-based differentiation methods
with SciPy's interpolation methods for computing derivatives of known test functions.
"""
...
def evaluate_methods(x, y, x_eval, true_derivative, noise_level=0.0, order=1):
    ...
    times['SciPy UnivariateSpline'] = time.time() - start_time
    results['SciPy UnivariateSpline'] = np.abs(scipy_spline_values - true_values)
    ...
    if order <= 3:  # CubicSpline only supports up to 3rd derivative
        ...
        results['SciPy CubicSpline'] = np.abs(scipy_cubic_values - true_values)
    else:
        results['SciPy CubicSpline'] = np.full_like(true_values, np.nan)
    times['SciPy CubicSpline'] = time.time() - start_time
...
def plot_comparison(test_name, func, deriv_func, x_range=(-5, 5), num_points=100, noise_level=0.05):
    ...
    # SciPy UnivariateSpline
    scipy_spline = UnivariateSpline(x, y_noisy, s=len(x)*noise_level**2)
    scipy_spline_values = scipy_spline.derivative(n=1)(x_eval)
    
    # SciPy CubicSpline
    cubic_spline = CubicSpline(x, y_noisy)
    scipy_cubic_values = cubic_spline.derivative(1)(x_eval)
    ...
    plt.plot(x_eval, scipy_spline_values, label='SciPy UnivariateSpline')
    plt.plot(x_eval, scipy_cubic_values, label='SciPy CubicSpline')
    plt.title(f'{test_name} - First Derivative Comparison')
    plt.legend()
    plt.grid(True)
    
    plt.tight_layout()
    plt.savefig(f'../output/{test_name.lower().replace(" ", "_")}_scipy_comparison.png')
    plt.close()
